...
class SimHash(object):
    ...
    def __init__(self, segfun=None, base=64):
       ...
       """
        ...
        if base is None or isinstance(
                base, int) is False or base % 32 != 0 and base <= 0:
            raise ValueError
        self.seg = segfun
    ...
    def figureprint(self, document):
        ...
        if document and isinstance(document, (str, unicode)):
            ...
            for hash_num, weight in hash_count_dict.items():  # 循环数组　权重
                self._hash_array_add(hash_array,
                                     self.get_array_by_weight(
                                         hash_num, weight))
    ...
    def _array_to_int(self, hash_array):
        ...
        if hash_array and isinstance(hash_array, (tuple, list)):
            return sum(1 << i for i in self.base_range
                       if hash_array[i] > 0) & self.LONG_MAX
    ...
    def get_array_by_weight(self, hash_num, weight):
        return [
            -weight if (hash_num & (1 << i)) == 0 else weight
            for i in range(64)
        ]
...
class WordSim(object):
    ...
    def __sim(self, var, child_num, length):
        return var * math.cos(child_num * math.pi / 180) * (
            float(child_num - length + 1) / child_num)
