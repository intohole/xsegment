...
class TfIdf(object):
    ...
    def load(self):
        if self.idf_file is None or not os.path.exists(
                self.idf_file) or not os.path.isfile(self.idf_file):
            return None
        d = defaultdict(float)
    ...
    def calc(self, doc, r="array", sort=True):
        ...
        tfidfDict = {
            word: tf * self.get_idf(word)
            for word, tf in wordCounter.items()
        }
        if r == "array":
            return sorted(
                tfidfDict.items(), key=lambda x: x[1],
                reverse=True) if sort is True else tfidfDict.items()
        else:
            return tfidfDict if sort is True else OrderedDict(
                sorted(tfidfDict.items(), key=lambda x: x[1], reverse=True))
